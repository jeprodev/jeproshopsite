...
if(isset($this->category_products) && $this->category_products){
...
$nbLi =  count($this->category_products);
...
<ul id="featured_products_block_center" class="product_list grid row" >
    ...
    $currentIndex = 0;
    foreach($this->category_products as $product){
        $totModulo = (count($this->category_products))%$nbItemsPerLine;
        $total = count($this->category_products);
        ...
        ?>
        <li class="ajax_block_product <?php if($currentIndex == 0){ ?>first_item<?php }elseif($currentIndex == ($nbLi-1)){ ?>last_item <?php }else{ ?>item<?php } if((($currentIndex+1)%$nbItemsPerLine) == 0){ ?>last_item_of_line<?php }elseif((($currentIndex +1)%$nbItemsPerLine) == 1){} if(($currentIndex+1) > ($nbLi - $totModulo)){ ?>last_line<?php } ?>">
            <div class="product_container" itemscope itemtype="http://schema.org/Product">
                ...
                <div class="right_block">
                    ...
                    <?php if (!$this->catalog_mode AND ((isset($product->show_price) && $product->show_price) || (isset($product->available_for_order) && $product->available_for_order))){ ?>
                        <div itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="content_price">
                            <?php if(isset($product->show_price) && $product->show_price && !isset($this->restricted_country_mode)){ ?>
                                ...
                                <?php if(isset($product->specific_prices) && $product->specific_prices && isset($product->specific_prices->reduction) && $product->specific_prices->reduction > 0){ ?>
                                    <?php /* {hook h="displayProductPriceBlock" product=$product type="old_price"} */ ?>
                                    <span class="old_price product_price"><?php echo JeproshopValidator::displayPriceWithoutReduction($product->price_without_reduction); ?></span>
                    ...
                    <?php if(isset($product->color_list)){ ?>
                        <div class="color_list_container"><?php echo $product->color_list; ?></div>
                    <?php }  ?>
                    <div class="product_flags">
                        <?php if(!$this->catalog_mode AND ((isset($product->show_price) && $product->show_price) || (isset($product->available_for_order) && $product->available_for_order))){
                            if(isset($product->online_only) && $product->online_only){ ?>
                                <span class="online_only"><?php echo JText::_('COM_JEPROSHOP_ONLINE_ONLY_LABEL'); ?> </span>
                            <?php }
                        }
                ...
                <?php /*if($page_name != ''){ ?>
                    <div class="functional_buttons clearfix">
                        ...
                        <?php if(isset($this->comparator_max_item) && $this->comparator_max_item){ ?>
                            <div class="compare">
                                <a class="add_to_compare" href="<?php echo JRoute::_($product->link); ?>" data-product-id="<?php echo $product->product_id; ?>"><?php echo JText::_('COM_JEPROSHOP_ADD_TO_COMPARE_LABEL'); ?> </a>
                            </div>
                        <?php }  ?>
                    </div>
                <?php } */  ?>
